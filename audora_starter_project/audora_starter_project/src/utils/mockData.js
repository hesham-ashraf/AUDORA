// Mock data for development and testing

export const albums = [
  {
    id: 1,
    title: "WILDFLOWER",
    artist: "Billie Eilish",
    coverUrl: "https://t2.genius.com/unsafe/344x344/https%3A%2F%2Fimages.genius.com%2Fb826bffa6a542a466c2143f4702b9f25.1000x1000x1.png",
    createdAt: "2025-04-15T10:30:00Z",
    tracks: [
      {
        id: 1,
        title: "WILDFLOWER",
        duration: 180,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      }
    ]
  },
  {
    id: 2,
    title: "After Hours",
    artist: "The Weeknd",
    coverUrl: "https://upload.wikimedia.org/wikipedia/en/c/c1/The_Weeknd_-_After_Hours.png",
    createdAt: "2025-04-10T14:20:00Z",
    tracks: [
      {
        id: 2,
        title: "After Hours",
        duration: 240,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054425/after_hours_ded5tr.mp3"
      }
    ]
  },
  {
    id: 3,
    title: "Stay",
    artist: "The Kid LAROI & Justin Bieber",
    coverUrl: "https://upload.wikimedia.org/wikipedia/en/0/0c/The_Kid_Laroi_and_Justin_Bieber_-_Stay.png",
    createdAt: "2025-04-08T09:15:00Z",
    tracks: [
      {
        id: 3,
        title: "Stay",
        duration: 210,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054389/Stay_ybim8r.mp3"
      }
    ]
  },
  {
    id: 4,
    title: "El Lilady",
    artist: "Amr Diab",
    coverUrl: "https://amrdiab.net/wp-content/uploads/2023/12/Makanak.jpg",
    createdAt: "2025-04-05T16:45:00Z",
    tracks: [
      {
        id: 4,
        title: "El Lilady",
        duration: 195,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746811912/Amr_Diab_..._El_Lilady_ou602n.mp3"
      },


      {
        id: 8,
        title: "Khaleek Maaya",
        duration: 195,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746812161/Amr_Diab_..._Khalik_Maaya_zucylt.mp3"
      }
    ]
  },
  {
    id: 5,
    title: "One of the Boys",
    artist: "Katy Perry",
    coverUrl: "https://people.com/thmb/n-FSsSYrhvu71Zpz9ZoVq1d1TFk=/750x0/filters:no_upscale():max_bytes(150000):strip_icc():focal(719x449:721x451):format(webp)/katy-perry-album-2-2b38f5dfb3804fcf94b6630e0f6591b0.jpg",
    createdAt: "2025-04-01T11:30:00Z",
    tracks: [
      {
        id: 5,
        title: "Hot N Cold",
        duration: 220,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054381/Katy_Perry_-_Hot_N_Cold_xksfuk.mp3"
      }
    ]
  },
  {
    id: 6,
    title: "Renaissance",
    artist: "Beyoncé",
    coverUrl: "https://upload.wikimedia.org/wikipedia/en/a/ad/Beyonc%C3%A9_-_Renaissance.png",
    createdAt: "2025-04-25T13:20:00Z",
    tracks: [
      {
        id: 6,
        title: "Break My Soul",
        duration: 215,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      }
    ]
  },
  {
    id: 7,
    title: "Midnights",
    artist: "Taylor Swift",
    coverUrl: "https://upload.wikimedia.org/wikipedia/en/9/9f/Midnights_-_Taylor_Swift.png",
    createdAt: "2025-04-22T10:15:00Z",
    tracks: [
      {
        id: 7,
        title: "Anti-Hero",
        duration: 200,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054425/after_hours_ded5tr.mp3"
      }
    ]
  }
];

export const podcasts = [
  {
    id: 1,
    title: "Deep Dive Talks",
    host: "Evan Talks",
    coverUrl: "https://images.unsplash.com/photo-1485579149621-3123dd979885?q=80&w=1931&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    createdAt: "2025-04-18T08:30:00Z",
    episodes: [
      {
        id: 1,
        title: "The Startup Grind",
        duration: 2400,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      },
      {
        id: 2,
        title: "AI in 2025",
        duration: 1800,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054425/after_hours_ded5tr.mp3"
      }
    ]
  },
  {
    id: 2,
    title: "Tech Talk Today",
    host: "Alex Chen",
    coverUrl: "https://images.unsplash.com/photo-1589903308904-1010c2294adc",
    createdAt: "2025-04-20T15:45:00Z",
    episodes: [
      {
        id: 3,
        title: "The Future of Web Development",
        duration: 1850,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054389/Stay_ybim8r.mp3"
      },
      {
        id: 4,
        title: "React vs Angular: The Complete Comparison",
        duration: 2100,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054386/Eternity_p08yrm.mp3"
      },
      {
        id: 5,
        title: "Building Scalable Applications",
        duration: 1700,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054381/Katy_Perry_-_Hot_N_Cold_xksfuk.mp3"
      }
    ]
  },
  {
    id: 3,
    title: "Health & Wellness Journey",
    host: "Dr. Sarah Johnson",
    coverUrl: "https://images.unsplash.com/photo-1511632765486-a01980e01a18",
    createdAt: "2025-04-12T11:20:00Z",
    episodes: [
      {
        id: 6,
        title: "Nutrition Myths Debunked",
        duration: 2250,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      },
      {
        id: 7,
        title: "Meditation for Beginners",
        duration: 1500,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054425/after_hours_ded5tr.mp3"
      }
    ]
  },
  {
    id: 4,
    title: "True Crime Stories",
    host: "Detective Mike Richards",
    coverUrl: "https://images.unsplash.com/photo-1551818176-60579e574b91",
    createdAt: "2025-04-14T09:30:00Z",
    episodes: [
      {
        id: 8,
        title: "The Mysterious Disappearance",
        duration: 3600,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054389/Stay_ybim8r.mp3"
      },
      {
        id: 9,
        title: "Cold Cases Solved",
        duration: 2700,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054386/Eternity_p08yrm.mp3"
      }
    ]
  },
  {
    id: 5,
    title: "Financial Freedom",
    host: "Emma Thompson",
    coverUrl: "https://images.unsplash.com/photo-1579621970563-ebec7560ff3e",
    createdAt: "2025-04-21T14:15:00Z",
    episodes: [
      {
        id: 10,
        title: "Investing for Beginners",
        duration: 2100,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054381/Katy_Perry_-_Hot_N_Cold_xksfuk.mp3"
      },
      {
        id: 11,
        title: "Building Wealth in Your 30s",
        duration: 1950,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      },
      {
        id: 12,
        title: "Crypto: Risk or Reward?",
        duration: 2400,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054425/after_hours_ded5tr.mp3"
      }
    ]
  },
  {
    id: 6,
    title: "Science Explained",
    host: "Prof. James Wilson",
    coverUrl: "https://images.unsplash.com/photo-1507413245164-6160d8298b31",
    createdAt: "2025-04-24T16:30:00Z",
    episodes: [
      {
        id: 13,
        title: "The Mystery of Black Holes",
        duration: 2800,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054389/Stay_ybim8r.mp3"
      },
      {
        id: 14,
        title: "How Vaccines Work",
        duration: 2200,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054386/Eternity_p08yrm.mp3"
      }
    ]
  },
  {
    id: 7,
    title: "The Travel Diaries",
    host: "Lisa Summers",
    coverUrl: "https://images.unsplash.com/photo-1488085061387-422e29b40080",
    createdAt: "2025-04-26T10:20:00Z",
    episodes: [
      {
        id: 15,
        title: "Hidden Gems of Southeast Asia",
        duration: 2500,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054381/Katy_Perry_-_Hot_N_Cold_xksfuk.mp3"
      },
      {
        id: 16,
        title: "Budget Travel in Europe",
        duration: 2300,
        audioUrl: "https://res.cloudinary.com/dnbk3iouw/video/upload/v1746054444/Billie_Eilish_-_WILDFLOWER_Official_Lyric_Video_evhsdd.mp3"
      }
    ]
  }
];

export const trendingContent = [
  ...albums.slice(0, 3),
  ...podcasts.slice(0, 2)
].sort(() => Math.random() - 0.5);

export const newReleases = [
  ...albums,
  ...podcasts
].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);

export const userData = {
  id: 1,
  name: "User",
  email: "user@audora.com",
  playlists: [
    {
      id: 1,
      title: "My Favorites",
      tracks: [albums[0].tracks[0], albums[1].tracks[0], albums[4].tracks[0]]
    },
    {
      id: 2,
      title: "Chill Mix",
      tracks: [albums[2].tracks[0], albums[3].tracks[0]]
    }
  ]
}; 